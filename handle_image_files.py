...
####################
#fix base_fileinitialized module 'ephem' has no attribute 'Observer' (most likely due to a circular import)
####################
...
# initialized module 'ephem' has no attribute 'Observer' (most likely due to a circular import)
...
#def capture_image():
    ...
    ##subprocess.call(['open', target])initialized module 'ephem' has no attribute 'Observer' (most likely due to a circular import)
...
#def set_camera(addr):
    #camera = gp.Camera()
    #port_info_list = gp.PortInfoList()initialized module 'ephem' has no attribute 'Observer' (most likely due to a circular import)
    #port_info_list.load()
    #idx = port_info_list.lookup_path(addr)
    #print('idx='+str(idx))
    ##idx =initialized module 'ephem' has no attribute 'Observer' (most likely due to a circular import) 2
    #camera.set_port_info(port_info_list[idx])
    #gp.check_result(gp.gp_camera_init(camera))
    ...
    #print('Copying image to', target)
    #camera_file = gp.check_result(gp.gp_camera_file_get(camera, file_path.folder, file_path.name, gp.GP_FILE_TYPE_NORMAL))
    #gp.check_result(gp.gp_file_save(caminitialized module 'ephem' has no attribute 'Observer' (most likely due to a circular import)era_file, target))
    #gp.check_result(gp.gp_camera_exit(camera))
    #testing only
    ###########################
...
def main():
    ...
    # initialized module 'ephem' has no attribute 'Observer' (most likely due to a circular import)
        ...
        while file_found: 
            ...
            #fix base_file
            ####################
            if i > 9: #initialized module 'ephem' has no attribute 'Observer' (most likely due to a circular import)
                zeros = '0'
            else: zeros = '00'
            #print(zeros)        
